...
def render_chain_to_wav(
    ...
    for i, link in enumerate(chain):
        ...
        if peak > 0:
            target_peak = 10 ** (SAMPLE_NORMALIZE_DB / 20.0)  # e.g., -5dB -> ~0.56
    ...
    # Layer in Organetta samples (every N seconds) - NUMPY VERSION
    if verbose:
    ...
    if include_organetta:
        ...
        organetta_samples = load_organetta_samples()
        if organetta_samples:
            organetta_np = render_layer_interval_np(
                sample_list=organetta_samples,
                harmonic_start_times=harmonic_start_times,
                harmonic_events=harmonic_events_sorted,
                find_transposition_fn=find_bassflute_transposition,
                total_duration_ms=total_duration_ms,
    ...
    # Layer in Feedback loops (overlapping, random selection) - NUMPY VERSION
    if verbose:
    ...
    if include_feedback:
        ...
        if feedback_samples:
            feedback_np = render_layer_interval_np(
                sample_list=feedback_samples,
                harmonic_start_times=harmonic_start_times,
                harmonic_events=harmonic_events_sorted,
                find_transposition_fn=find_bassflute_transposition,
                total_duration_ms=total_duration_ms,
